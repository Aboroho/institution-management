...
export default function TeacherNoticesPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR(`teacher-notices${query}`, () => get<Row[]>(`/notices${query}`));
  ...
  return (
    <div>
      ...
      <PageHeader title="Notices" subtitle="Send notices to admins, your assigned offerings, or permitted students." actions={<Button onClick={() => { setActionError(""); setComposer({ mode: "create" }); }}><Plus size={16} /> New notice</Button>} />
      <Card className="mb-4 p-4">
        <div className="flex flex-wrap items-end gap-3">
          <div className="min-w-[240px] flex-1">
            <label htmlFor="teacher-notice-search" className="mb-1 block text-sm font-medium text-slate-700">Search notices</label>
            <Input id="teacher-notice-search" value={search} onChange={(event) => { setSearch(event.target.value); setPage(1); }} placeholder="Search title or message..." />
          </div>
          <div className="min-w-[280px] flex-1">
            <Label>Course offering</Label>
            <SearchableSelect options={offerings} value={courseOfferingId} onChange={(value) => { setCourseOfferingId(value); setPage(1); }} clearLabel="All assigned offerings" />
          </div>
      ...
      {isLoading ? <LoadingSkeleton /> : error ? <ErrorState message="Failed to load notices" onRetry={() => mutate()} /> : items.length === 0 ? (
        <EmptyState title="No notices" hint="Publish an announcement to one of your permitted audiences." action={<Button onClick={() => setComposer({ mode: "create" })}><Plus size={16} /> New notice</Button>} />
      ) : (
        <>
                ...
                <Card key={str(notice.id)} className="p-4">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      <Link href={`/teacher/notices/${str(notice.id)}`} className="flex items-center gap-2 text-lg font-semibold text-slate-900 hover:text-brand-700 hover:underline">
                      ...
                      <div className="mt-3 flex flex-wrap items-center gap-2 text-xs text-slate-500">
                        <span>{notice.canEdit === true ? "Created by you" : `From ${str(creator.name) || "EMS"}`}</span>
                        <span>·</span>
